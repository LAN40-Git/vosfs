...
auto vosfs::rpc::detail::SessionManager::assign_auth_session(const std::string& id, kosio::net::TcpStream&& stream,
    kosio::net::SocketAddr addr) -> std::shared_ptr<Session> {
    auto new_session = std::make_shared<Session>(true, id, std::move(stream), addr);
    auto copy_session = new_session;
