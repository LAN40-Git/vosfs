...
class VosfsClient : public BaseClient<VosfsClient> {
    friend class BaseClient<VosfsClient>;
public:
    explicit VosfsClient(std::string_view server_ip, uint16_t server_port)
        : BaseClient<VosfsClient>(server_ip, server_port) {}
...
auto main_loop() -> kosio::async::Task<void> {
    auto auth_client = VosfsClient{"127.0.0.1", 8080};
    int n{1};
    while (n > 0) {
        --n;
